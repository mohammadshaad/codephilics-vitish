...
list_of_vehicles = ["bicycle","car","motorbike","bus","truck", "train","person","number plate"]
	...
	if len(idxs) > 0:
		...
		for i in idxs.flatten():
			...
			if (LABELS[classIDs[i]] in list_of_vehicles):
				...
				crop_img = frame[y:yh,x:xw]

				if(len(crop_img[0]) == 0):
					break

				if(LABELS[classIDs[i]] == 'car'):
					path = "/Users/paramkansagra/Downloads/Yolo-Vehicle-Counter-master/car/"+str(ID)+".jpg"
					if(os.path.exists(path)):
						img = cv2.imread(path)
						heightOld = img.shape[0]
						widthOld = img.shape[1]

						if(heightOld < h and widthOld < w):
							os.remove(path)
							cv2.imwrite(os.path.join(path), crop_img)
					else:		
						cv2.imwrite(os.path.join(path), crop_img)
				
				if(LABELS[classIDs[i]] == 'number_plate'):
					path = "/Users/paramkansagra/Downloads/Yolo-Vehicle-Counter-master/number_plate/"+str(ID)+".jpg"
					if(os.path.exists(path)):
						img = cv2.imread(path)
						heightOld = img.shape[0]
						widthOld = img.shape[1]
				...
				if(LABELS[classIDs[i]] == 'motorbike'):
					path = "/Users/paramkansagra/Downloads/Yolo-Vehicle-Counter-master/moterbike/"+str(ID)+".jpg"
					if(os.path.exists(path)):
						img = cv2.imread(path)
						heightOld = img.shape[0]
						widthOld = img.shape[1]
				...
				if(LABELS[classIDs[i]] == 'truck'):
					path = "/Users/paramkansagra/Downloads/Yolo-Vehicle-Counter-master/truck/"+str(ID)+".jpg"
					if(os.path.exists(path)):
						img = cv2.imread(path)
						heightOld = img.shape[0]
						widthOld = img.shape[1]
				...
				if(LABELS[classIDs[i]] == 'bus'):
					path = "/Users/paramkansagra/Downloads/Yolo-Vehicle-Counter-master/bus/"+str(ID)+".jpg"
					if(os.path.exists(path)):
						img = cv2.imread(path)
						heightOld = img.shape[0]
						widthOld = img.shape[1]
				...
				if(LABELS[classIDs[i]] == 'person'):
					path = "/Users/paramkansagra/Downloads/Yolo-Vehicle-Counter-master/person/"+str(ID)+".jpg"
					if(os.path.exists(path)):
						img = cv2.imread(path)
						heightOld = img.shape[0]
						widthOld = img.shape[1]
